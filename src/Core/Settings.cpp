...
Write fully qualified URIs into Iceberg metadata files instead of storage-relative paths, so external readers can resolve them: `s3://bucket/key`, or `file:///path` for an absolute local path.
Only applied when the table is created; existing tables keep the form their metadata already declares.
)", EXPERIMENTAL) \
