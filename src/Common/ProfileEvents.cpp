...
The server successfully detected this situation and will download merged part from the replica to force the byte-identical result.
    ...
    M(AdaptiveAggregationSpillBacklogSheds, "How many times the adaptive aggregation shed the staged backlog because a thread the thaw put back on the baseline algorithm was about to spill while records staged before the thaw were still resident.", ValueType::Number) \
