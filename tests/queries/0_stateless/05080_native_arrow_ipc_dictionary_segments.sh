    ...
    for shape, value_type, value, ch_type, default in (
        ...
        for nullable in (False, True):
            ...
            result = subprocess.run(local + [
                ...
                f"--allow_experimental_nullable_tuple_type={int(shape == 'nullable_tuple')}",
                "--query", f"SELECT ifNull(toJSONString(c), 'null') FROM file('{path}', '{fmt}', 'c {ch_type}') FORMAT TSVRaw",
            ], text=True, capture_output=True)
            assert result.returncode == 0, result.stderr
            actual = [json.loads(line) for line in result.stdout.splitlines()]
            assert actual == expected, (fmt, shape, nullable, actual, expected)
            print(f"OK {fmt} {shape} {'nullable' if nullable else 'non-nullable'}")
PY
